...
import { getAIBills } from '../data/aiBillsTracker.js'
 ...
 *
 * @param {Array} scrapedCandidates - output of scraper functions
...
export async function syncCandidates(scrapedCandidates) {
  ...
  for (const scraped of scrapedCandidates) {
    ...
    // Compute data hash for this scraped candidate
    const dataHash = scraped.dataHash ?? computeDataHash({
      known_positions:      scraped.knownPositions ?? null,
      fossil_fuel_donations: scraped.fossilFuelDonations ?? null,
      incumbent_status:     scraped.incumbentStatus ?? null,
    })
    ...
    if (!match) {
      ...
      const { error } = await supabase.from('candidates').insert({
        ...
        // New fields for House candidates
        district:             scraped.district ?? null,
        district_pvi:         scraped.districtPvi ?? null,
        is_battleground:      scraped.isBattleground ?? null,
        tier:                 scraped.tier ?? null,
        data_hash:            dataHash,
      })
      ...
      // Update House-specific fields if missing from existing record
      if (scraped.district && !match.district) {
        updates.district = scraped.district
      }
      if (scraped.isBattleground != null && match.is_battleground == null) {
        updates.is_battleground = scraped.isBattleground
      }
      if (scraped.tier != null && match.tier == null) {
        updates.tier = scraped.tier
      }

      // Check if the candidate's data has changed (triggers re-analysis)
      const existingHash = match.data_hash
      if (existingHash && existingHash !== dataHash) {
        console.log(`[sync] Data changed for ${match.name} — clearing analysis for re-run`)
        updates.data_hash = dataHash
        updates.climate_score = null
        updates.climate_analysis = null
        updates.issue_tags = null
        // Flag for AI policy re-score as well
        updates.ai_policy_score = null
        updates.ai_score_updated_at = null
        summary.aiDataUpdated.push(match.name)
  ...
  // --- AI data refresh for Senate and incumbent House candidates ---
  const aiEligible = existing.filter(c =>
    c.office_type === 'us_senate' ||
    (c.office_type === 'us_house' && c.incumbent_status === 'incumbent')
  )

  console.log(`[sync] Refreshing AI policy data for ${aiEligible.length} Senate/incumbent candidates…`)

  for (const candidate of aiEligible) {
    try {
      const [bigTechResult, aiBills] = await Promise.all([
        getBigTechDonations(candidate.id, candidate.name, candidate.state),
        getAIBills(candidate.name, candidate.state),
      ])

      // Flag if Big Tech donation level changed
      const prevLevel = candidate.big_tech_donation_level
      if (prevLevel && prevLevel !== bigTechResult.level) {
        summary.bigTechChanges.push(`${candidate.name}: ${prevLevel} → ${bigTechResult.level}`)
        // Clear AI score so it gets re-run with updated data
        await supabase.from('candidates').update({
          ai_policy_score:    null,
          ai_score_updated_at: null,
        }).eq('id', candidate.id)
        summary.aiDataUpdated.push(candidate.name)
      }

      console.log(`[sync] AI data: ${candidate.name} — BigTech: ${bigTechResult.level}, AI bills: ${aiBills.length}`)
    } catch (err) {
      console.warn(`[sync] AI data fetch failed for ${candidate.name}:`, err.message)
    }
  }
  ...
  console.log(`  ${summary.aiDataUpdated.length} candidates flagged for AI re-score`)
  if (summary.bigTechChanges.length) {
    console.log(`  Big Tech donation changes: ${summary.bigTechChanges.join(', ')}`)
  }
