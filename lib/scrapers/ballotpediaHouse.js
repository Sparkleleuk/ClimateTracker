...
const HOUSE_2026_URL = `${BASE_URL}/United_States_House_of_Representatives_elections,_2026`
...
function parseDistrictUrl(url) {
  try {
    ...
    const match = decoded.match(
      /elections_in_([^']+)'s_(.+?)_congressional_district,?_?2026/i
    )
...
async function scrapeDistrictUrls() {
  ...
  $('a[href*="House_of_Representatives_elections_in_"]').each((_, el) => {
...
export async function scrapeBallotpedia2026House() {
  ...
  // Only scrape battleground districts to stay within Vercel's timeout limits.
  // Non-battleground candidates are added via direct Ballotpedia URLs when needed.
  const filteredDistricts = new Map(
    [...districtUrls.entries()].filter(([key]) => BATTLEGROUND_DISTRICTS.has(key))
  )
  console.log(`[house] Scraping ${filteredDistricts.size} battleground districts`)

  const allCandidates = []
  let scraped = 0

  for (const [key, { url, stateName, stateAbbr, district }] of filteredDistricts) {
    try {
